 ...
 *
 * The source consists of 5 WGSP tables, 1 PCP_GEOCODES target table
...
class ProviderOperation(configPath: String, env: String, queryFileCategory: String)
  ...
  override def transformData(inDFs: Map[String, DataFrame]): Map[String, DataFrame] = {
    ...
    val pcp_pcpaltid_joinDF = wgspPcpDF.join(wgspPcpAltidDF,
      Seq("prov_pcp_id", "hmo_type_cd", "src_sor_cd", "pcp_efctv_dt"), "left")
    ...
    val pcpasp_spclity_joinDF = wgspPcpAspDF.join(wgspPcpSpcltyCdMappingDF, Seq("wgs_spclty_cd"))
      .select($"spclty_cd", $"wgs_spclty_cd",
    ...
    //Step 7: Defaulting the pcp_ranking_id to 0 
    ...
    val wgspJoinDF_tmp = pcpAltidPcpAspSpclty_cntrctCpCdMapping_pcpGeocodes_hcaRanking_joinDF.
      join(provLang_LangMapping_joinDF, Seq("prov_pcp_id", "hmo_type_cd", "src_sor_cd", "pcp_efctv_dt"), "left")
    ...
    val wgspJoinDF = wgspJoinDF_tmp.join(provLang_LangMapping_grouped_joinDF, Seq("prov_pcp_id"), "left")
    ...
    val finalProvDF = provVBP_GroupedDF.join(provVBPDF,
      ...
      .select(provVBP_GroupedDF("prov_pcp_id"), $"grpg_rltd_padrs_efctv_dt", $"grpg_rltd_padrs_trmntn_dt", 
          $"adrs_zip_cd", $"adrs_zip_plus_4_cd",$"latd_cordnt_nbr", $"lngtd_cordnt_nbr", 
          $"adrs_cnty_cd", $"pcp_rankg_id", provVBP_GroupedDF("rgnl_ntwk_id"), provVBP_GroupedDF("spclty_cd"),
        provVBP_GroupedDF("wgs_spclty_cd"), provVBP_GroupedDF("spclty_desc"), provVBP_GroupedDF("spclty_mnemonics"), 
        provVBP_GroupedDF("prmry_spclty_ind"),$"max_mbr_cnt", $"curnt_mbr_cnt", $"cp_type_cd", 
        $"acc_new_patient_flag",
        when($"iso_3_cd" === null || $"iso_3_cd" === "" ||provVBPDF.col("iso_3_cd").isNull || $"iso_3_cd" === "null", 
            lit("ENG")).otherwise($"iso_3_cd").as("iso_3_cd"),
       when($"pcp_lang" === null || $"pcp_lang" === "" ||provVBPDF.col("pcp_lang").isNull || $"pcp_lang" === "null", 
            lit("ENGLISH")).otherwise($"pcp_lang").as("pcp_lang"),
        $"vbp_flag",
    ...
    if (sendNtfctnMail) {
      val minRowCnt = config.getInt("providerDataMinCnt")
      val maxRowCnt = config.getInt("providerDataMaxCnt")
      ...
      if ((finalDfCnt < minRowCnt) || (finalDfCnt > maxRowCnt)) {
        ...
        throw new Exception("[SPCP-ETL] : PROVIDER_INFO Data Load Suspended. Row count beyond threshold.")
      } else {
