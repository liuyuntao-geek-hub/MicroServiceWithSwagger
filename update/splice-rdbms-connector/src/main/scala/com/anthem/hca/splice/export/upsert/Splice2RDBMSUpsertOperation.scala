...
class Splice2RDBMSUpsertOperation(configPath: String,env: String,tableName: String)
  ...
  //Appending hash column to both dataframes.
    ...
    val spliceTbl_sqlTbl_joinDF = spliceTblMD5.join(sqlTblMD5,
      ...
      "FULL_OUTER")
    info("Done joining SQL and Splice tables")
    spliceTbl_sqlTbl_joinDF.cache()
    info("Count is " + spliceTbl_sqlTbl_joinDF.count())
    spliceTbl_sqlTbl_joinDF.cache()
    ...
    info("Formed deleteDF")
    deleteDF.cache()
    info("Count is " + deleteDF.count())
    ...
    info("Formed insertDF")
    insertDF.cache()
    info("Count is " + insertDF.count())
    ...
    info("Formed sameRowsDF")
    sameRowsDF.cache()
    info("Count is " + sameRowsDF.count())
    ...
    info("Formed updtdDf2")
    updtdDf2.cache()
    info("Count is " + updtdDf2.count())
