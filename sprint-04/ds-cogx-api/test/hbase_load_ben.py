...
df = pd.read_csv("/home/af08853/benefits_reference_new.txt")
print(df.dtypes)
df['START_DT'] = df['START_DT'].str.replace('/','')
df['END_DT'] = df['END_DT'].str.replace('/','')
print(df.dtypes)
#df['START_DT'] = df['START_DT'].dt.strftime('%m%d%Y')
...
for key in keys:
    ...
    val = {}
    #print dict[key]
    val['cogxbenefit'] = dict[key] 
    val = str(json.dumps(val))
    print val
    mutations = [Hbase.Mutation(column='c1:jsonData', value= val)]
    client.mutateRow('dv_hb_ccpcogx_nogbd_r000_in:cogx_benefits_v', key_1, mutations,None)
