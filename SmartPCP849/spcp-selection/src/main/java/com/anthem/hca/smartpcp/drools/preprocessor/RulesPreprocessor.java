				...
				for (int rowIndex = firstDataRow; rowIndex <= sheet.getLastRowNum(); rowIndex++) {
					...
					if(row != null) {
						...
						if(agendaGroupCell != null) {
							...
							createMatrixColumn("LOB", agendaGroup, row.getCell(1).getStringCellValue().replaceAll(RulesRegex.DOUBLE_QUOTES, ""));
							createMatrixColumn("MARKET", agendaGroup, row.getCell(2).getStringCellValue().replaceAll(RulesRegex.DOUBLE_QUOTES, ""));
							createMatrixColumn("PRODUCT", agendaGroup, row.getCell(3).getStringCellValue().replaceAll(RulesRegex.DOUBLE_QUOTES, ""));
							createMatrixColumn("ASSIGNMENT_TYPE", agendaGroup, row.getCell(4).getStringCellValue().replaceAll(RulesRegex.DOUBLE_QUOTES, ""));
							createMatrixColumn("ASSIGNMENT_METHOD", agendaGroup, row.getCell(5).getStringCellValue().replaceAll(RulesRegex.DOUBLE_QUOTES, ""));
						}
					}
				}
			} finally {
