...
public class SmartPCPService {
	...
	public OutputPayload getPCP(Member member) {
		...
		LOGGER.info("Updating into operations Audit ");
		int update = auditService.updateOperationFlow(outputPayload, operationsAudit, member);
		LOGGER.info("Updated into operations Audit ");
		if (update < 1) {
	...
	private OutputPayload getPCPAM(Member member, OperationsAuditUpdate operationsAudit) throws Exception {
		...
		if (null != validPCP) {
			...
			return outputPayloadHelper.createErrorPayload(ErrorMessages.NO_VALID_PCP, ResponseCodes.NO_VALID_PCP, member, 1);
