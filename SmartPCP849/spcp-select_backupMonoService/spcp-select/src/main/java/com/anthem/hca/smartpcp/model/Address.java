...
public class Address {
	...
	@NotBlank(message = ErrorMessages.MISSING_ADRS_LINE1,groups=IBlankGroup.class)
	@Size(max = 30, message = ErrorMessages.INVALID_ADRS_LINE1,groups=ISizeGroup.class)
	@ApiModelProperty(required = true, dataType = "String", notes = "Contains address line2 value,max size = 30")
	private String addressLine1;

	@Size(max = 30, message = ErrorMessages.INVALID_ADRS_LINE2,groups=ISizeGroup.class)
	@ApiModelProperty(dataType = "String", notes = "Contains address line2 value,max size = 30")
	private String addressLine2;

	@NotBlank(message = ErrorMessages.MISSING_CITY,groups=IBlankGroup.class)
	@Size(max = 13, message = ErrorMessages.INVALID_CITY,groups=ISizeGroup.class)
	@Pattern(regexp = "[[a-z-A-Z]\\s]*", message = ErrorMessages.INVALID_CITY,groups=IRegexGroup.class)
	@ApiModelProperty(required = true, dataType = "String", notes = "Contains city ,max size = 13 and regular exp as [[a-z-A-Z]\\s]*")
	private String city;

	@NotBlank(message = ErrorMessages.MISSING_STATE,groups=IBlankGroup.class)
	@Size(max = 2, message = ErrorMessages.INVALID_STATE,groups=ISizeGroup.class)
	@Pattern(regexp = "[a-z-A-Z]*", message = ErrorMessages.INVALID_STATE,groups=IRegexGroup.class)
	@ApiModelProperty(required = true, dataType = "String", notes = "Contains state,max size = 2 and regular exp as [a-z-A-Z]*")
	private String state;

	
	@NotBlank(message = ErrorMessages.MISSING_ZIPCODE,groups=IBlankGroup.class)
	@Pattern(regexp = "^(?!0{5})[0-9]{5}$", message = ErrorMessages.INVALID_ZIPCODE,groups=IRegexGroup.class)
	@ApiModelProperty(required = true, notes = "Contains zipcode,Should be 5 digit number")
	private String zipCode;

	@Size(max = 4, message = ErrorMessages.INVALID_ZIPFOUR,groups=ISizeGroup.class)
	@ApiModelProperty(notes = "Contains zip plus4,max size = 4")
	private String zipFour;

	@Size(max = 3, message = ErrorMessages.INVALID_COUNTY_CODE,groups=ISizeGroup.class)
