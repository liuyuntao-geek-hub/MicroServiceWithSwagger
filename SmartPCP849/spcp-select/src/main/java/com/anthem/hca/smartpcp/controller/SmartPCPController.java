...
import com.anthem.hca.smartpcp.service.AsyncService;
import com.anthem.hca.smartpcp.service.OperationAuditService;
import com.anthem.hca.smartpcp.service.OutputPayloadService;
import com.anthem.hca.smartpcp.service.SmartPCPService;
import com.anthem.hca.smartpcp.validator.IGroupSequenceForMember;
 ...
 * information as request and returns the OutputPayload containing the assigned
 * pcp id, module through which the pcp was assigned, response code and message.
 * 
 * @author AF71111
 */
@RestController
...
public class SmartPCPController {
	...
	public OutputPayload getPCP(@ApiParam(value = "Member input Payload", required = true) @RequestBody Member member) {
		...
		try {
			asyncService.asyncMemberInsert(member, dateTime);
			Validator validator = validatorFactory.getValidator();
			Set<ConstraintViolation<Member>> violations = validator.validate(member, IGroupSequenceForMember.class);
			if (violations.isEmpty()) {

				outputPayload = service.getPCP(member, dateTime);
				...
				if (iterator.hasNext()) {
					...
					LOGGER.error("Input Validation Failed {}", constraint.getMessage());
					outputPayload =  payloadHelper.createErrorPayload(constraint.getMessage(), ResponseCodes.OTHER_EXCEPTIONS,
							member, 1);
				}
				
				int result = auditService.updateOperationFlow(outputPayload, new OperationsAuditUpdate(), new Member());
				if (result < 1) {
					outputPayload = payloadHelper.createErrorPayload(ErrorMessages.AUDIT_TX_UPDATE_FAILURE,
	...
	public OutputPayload handleException(JsonProcessingException exception) {
		...
		outputPayload = payloadHelper.createErrorPayload(exception.getClass().getSimpleName(), ResponseCodes.OTHER_EXCEPTIONS,
				null, 1);
		int result = auditService.updateOperationFlow(outputPayload, new OperationsAuditUpdate(), new Member());
		if (result < 1) {
			outputPayload = payloadHelper.createErrorPayload(ErrorMessages.AUDIT_TX_UPDATE_FAILURE,
					ResponseCodes.OTHER_EXCEPTIONS, new Member(), 1);
		}
