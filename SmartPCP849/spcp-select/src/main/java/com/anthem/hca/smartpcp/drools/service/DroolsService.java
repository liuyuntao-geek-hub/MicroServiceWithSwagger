...
public class DroolsService implements ApplicationListener<EnvironmentChangeEvent> {
	...
	private String businessRuleFileRestUrl;
    ...
    public void onApplicationEvent(EnvironmentChangeEvent event) {
		...
		businessRuleFileRestUrl = environment.getProperty("spcp.business.rules.url");
	 ...
	 * @see    Resource
	 */
    private List<ResponseEntity<Resource>> getRuleFiles() throws IOException {
    	List<ResponseEntity<Resource>> ruleData = new ArrayList<>();

    	String msg = null;
		if (fileUrls != null) {
			String token = oAuthAccessTokenConfig.fetchAccessToken();
			HttpHeaders headers = oAuthAccessTokenConfig.buildHttpHeadersWithAccessToken(token);
	    	HttpEntity<String> request = new HttpEntity<>(headers);
			for(int fileIndex = 0; fileIndex < fileUrls.size(); fileIndex++) {
		    	ResponseEntity<Resource> fileStream = restTemplate.exchange(
		    			businessRuleFileRestUrl + "/file/" + fileUrls.get(fileIndex),
		    			HttpMethod.GET,
		    			request,
		    			Resource.class
		    	);

				if (fileStream.getBody() == null) {
					msg = StaticLoggerMessages.CONTENT_ERROR + fileUrls.get(fileIndex);
					logger.error(msg);
					throw new IOException(msg);
				}
				else {
					ruleData.add(fileStream);
    ...
    private KieFileSystem getKieFileSystem() throws IOException {
        ...
        int fileIndex = 0;
        List<ResponseEntity<Resource>> downloadedFiles = getRuleFiles();
        
     // Select the files required for creating the Matrix
        for (ResponseEntity<Resource> data: downloadedFiles) {
        	String fileName = fileUrls.get(fileIndex++);
        	if (matrixFileUrls.contains(fileName)) {
        		matrixFiles.add(data.getBody().getInputStream());
        	}
        }
     ...
     // Write all the files to Kie FileSystem that were downloaded from the Business Rules service
        fileIndex = 0;
        for (ResponseEntity<Resource> data: downloadedFiles) {
			org.kie.api.io.Resource resource = ResourceFactory.newInputStreamResource(data.getBody().getInputStream());
			kieFileSystem.write(kieFileLocation + fileUrls.get(fileIndex++), resource);
        }
