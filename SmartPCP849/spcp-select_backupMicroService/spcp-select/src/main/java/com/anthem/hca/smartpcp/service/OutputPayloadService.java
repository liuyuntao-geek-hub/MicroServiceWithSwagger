...
public class OutputPayloadService {
	...
	public OutputPayload createErrorPayload(String tranErrorText, String tranErrorCode, Member member, int totalErrors) {
		...
		if(ResponseCodes.SERVICE_NOT_AVAILABLE == tranErrorCode){
	 ...
	 * @return OutputPayload
	 */
	public OutputPayload createSuccessPaylod(String reportingCode,String reportingText, Member member, String pcpId, boolean dummyFlag,
			String networkId, Integer pcpScore) {
		...
		reporting.setReportingCode(reportingCode);
		reporting.setReportingText1(reportingText);
		status.setTransaction(transactionStatus);
		status.setService(serviceStatus);
		BeanUtils.copyProperties(member, memberOutput);
		BeanUtils.copyProperties(member.getAddress(), address);
		...
		if (dummyFlag) {
			...
			dummyPCP.setProvPcpId(pcpId);
			dummyPCP.setDummyPCP(Constants.TRUE);
			output.setProvider(dummyPCP);
		} else {
			
			PCP pcp = providerRepo.getAssignedPcp(pcpId, networkId);
			
			if (reportingCode.equalsIgnoreCase(Constants.REPORTING_CODE_MDO)) {
				pcp.setPcpScore(pcpScore);
			} else {
				pcp.setPcpScore(0);
				pcp.setPcpRank(0);
			}
			output.setProvider(pcp);
			asyncService.asyncUpdate(pcpId, networkId);
			
		}
		LOGGER.info("PCP Assigned provPcpId={}, network={}, dummy={}", pcpId, networkId, dummyFlag);
		output.setSmartPCPTraceID(tracer.getCurrentSpan().traceIdString());
		return output;
	}

	@Bean
	public ProviderInfoRepo providerInfoRepo() {
