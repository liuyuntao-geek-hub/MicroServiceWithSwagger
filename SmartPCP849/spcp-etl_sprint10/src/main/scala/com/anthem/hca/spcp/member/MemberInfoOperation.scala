 ...
 */
  override def extractData(): Map[String, DataFrame] = {
    ...
    val mapDF = Map("memProfEnr" -> memProfEnrDF, "mem" -> memDF, "xwalk" -> xwalkDF, "wgspPcp" -> wgspPcpDF, "wgspPcpAltid" -> wgspPcpAltidDF, "piAfntyAtrbn" -> piAfntyAtrbnDF, "spcpThrshldTbl" -> spcpThrhldDF)
  ...
  override def transformData(inDFs: Map[String, DataFrame]): Map[String, DataFrame] = {
    ...
    val minRowCnt: Integer = spcpThrshldDF.filter($"table_nm" === memberTargetTable).select($"min_cnt").head().getInt(0)
    val maxRowCnt: Integer = spcpThrshldDF.filter($"table_nm" === memberTargetTable).select($"max_cnt").head().getInt(0)
    ...
    val wgspPcpPcpalitJoinDF = wgspPcpDF.withColumnRenamed(
      "TAX_ID",
      "PROV_ORG_TAX_ID").join(wgspPcpAltidDF.withColumnRenamed("NPI", "IP_NPI"), Seq("PROV_PCP_ID", "HMO_TYPE_CD", "SRC_SOR_CD", "PCP_EFCTV_DT")).select($"PROV_ORG_TAX_ID", $"IP_NPI", $"PROV_PCP_ID").distinct()
    ...
    val memberFinalDFWithSelectColumns = memberFinalDF.select("MBR_KEY", "HC_ID", "FRST_NM", "LAST_NM", "MID_INIT_NM", "BRTH_DT", "MCID", "MBR_SQNC_NBR",
    ...
    if ((finalDfCnt < minRowCnt) || (finalDfCnt > maxRowCnt)) {
      ...
      println("Member data not within threshold limits")
