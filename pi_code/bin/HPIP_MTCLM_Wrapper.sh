#!/bin/bash
#=================================================================================================================================================
# Title            : HPIP_MTCLM_WRAPPER
# ProjectName      : PADP - PI Data (HPIP)
# Filename         : HPIP_MTCLM_Wrapper.sh
...
#==================================================================================================================================================== 
## SAMPLE ARGUMENTS
#subject_area='HPIP_MTCLM'
#=====================================================================================================================================================

now=$(date +"%Y%m%d%H%M%S")
echo $now

echo "MTCLM wrapper triggered at $now"
...
spark-submit --master yarn --deploy-mode cluster --name $SUBJECT_AREA --num-executors 40 --executor-memory 30G --executor-cores 4 --driver-memory 16G --files $CONFIG_PATH/log4j.xml --driver-java-options "-Dlog4j.configuration=log4j.xml" --conf "spark.executor.extraJavaOptions=-Dlog4j.configuration=log4j.xml" --conf spark.yarn.executor.memoryOverhead=8192 --conf spark.network.timeout=600 --conf spark.driver.maxResultSize=2g --class com.anthem.hpip.mtclm.MTCLMDriver $JAR_FILE_PATH/$JAR_NAME $CONFIG_PATH $ENV $PROP_FILE
