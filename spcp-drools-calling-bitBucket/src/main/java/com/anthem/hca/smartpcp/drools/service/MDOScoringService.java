...
public class MDOScoringService implements IRulesService {
	...
	private static final Logger logger = LogManager.getLogger(MDOScoringService.class);

	@Override
	public MDOScoringRules getRules(RulesInputPayload payload) throws DroolsParseException, JsonProcessingException {
		boolean fallback = droolsService.isFallbackRequired(payload);
		logger.info("Fallback Required = " + fallback);

		MDOScoringRules rules = (MDOScoringRules)droolsService.fireRulesFor(AgendaGroup.MDO_SCORING, payload, fallback);
		clientService.insertOperationFlow(AgendaGroup.MDO_SCORING.getValue());
