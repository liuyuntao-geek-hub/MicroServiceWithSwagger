...
public class ProviderValidationRulesTest {
	...
	@Test
	public void testIsRolloverRequired() throws DroolsParseException {
		rules.setRolloverRequired("YES");
		assertTrue(rules.isRolloverRequired());

		rules.setRolloverRequired("no");
		assertFalse(rules.isRolloverRequired());
	}

	@Test
	public void testIsRolloverRequiredThrowsException() {
		try {
			rules.setRolloverRequired(null);
			fail("Expected an DroolsParseException to be thrown");
		} catch (DroolsParseException dpe) {
			assertThat(dpe.getMessage(), containsString("cannot be empty"));
		}
