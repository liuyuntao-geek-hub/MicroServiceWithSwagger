...
class computeUM():
    ...
    def request_transform(self, payload):
        mapping = copy.deepcopy(self.app.config['mappings']['um_mapping'])
    ...
    def __UM_validate__(self, payload):
        ...
        print(type(payload))
        payload_list = []
        if(type(payload) == "<class 'dict'>"):
            payload_list = [payload]
        elif (type(payload) == "<class 'list'>"):
            for item in payload:
                payload_list.append(item)
        resp = json_transform.assert_valid_schema(payload_list,'uber_um_payload.json','src-lib')
        #need to check response object error_code if fine than we need to return true else raise exception to @app.errorhandler(error_code)
        #we need to define error handler in app.py or in computeUM.py
        return True

    def process_request(self, payload):
        if (self.__UM_validate__(payload) == True):
            self.logger.debug("Valid", extra=self.extra)
        else:
            self.logger.debug("Discard", extra=self.extra)

        UMtransformed_payload = self.request_transform(payload)
        ...
        self.logger.debug(type(final_out['Top_1_UM']), extra=self.extra)
