...
Step 2 - Run com.anthem.hpip.Template.TemplateDriver within eClipse with the following Parameter

	C:\java\git\repos\coe_2.0\conf local TemplateLocal

Step 3 - Before to deploy to cluster: 
...
Step 4 - pom.xml need to be updated when running on local (The first time )
	When using provided build using maven on local: the following file
---------------------------
import org.apache.hadoop.fs.FileSystem
			- get not found error
---------------------------
Need to change from: (Works if build to deploy - provided)
---------------------------
<dependency>
...
---------------------------
to: (Works if build on local - compile)
-----------------------
<dependency>
	<groupId>org.apache.spark</groupId>
	<artifactId>spark-core_${scala.tools.version}</artifactId>
...
Step 5 - When running program on local: get error:
-----------------------------------------------------
...
http://public-repo-1.hortonworks.com/hdp-win-alpha/winutils.exe
              To:
C:\java\installation\hadoopWin\winutils\bin\winutils.exe
------
              - this will set up the Hadoop on your local environment

Action 2 - Setup Windows environment variable:

HADOOP_HOME=C:\java\installation\hadoopWin\winutils

C:\java\installation\hadoopWin\winutils
================================================================
...
Step 1 - Change the code in com.anthem.hpip.config.Spark2Config 
	- pick up the remote run section 
	
Step 2 - if needed change the pom.xml => use the provided section => this make the jar more effective
...
---------------------------------------------------------------
Step 4 - FTP data file: testdata_combined.csv
From:
...
---------------------------------------------------------------
Step 5 - FTP CenterOfExcellence-2.0.0.jar
From:
...
---------------------------------------------------------------
Step 5 - FTP query_smartProvider.properties
From:
...
---------------- Works but on cluster has error -----------------------
Application application_1527038069742_3015 failed 1 times due to AM Container for appattempt_1527038069742_3015_000001 exited with exitCode: 0 
- it might be caused by single thread read from csv file
- Maybe because only Driver is used during this proess
-------------------------------------------------------------------------

spark2-submit \
--master yarn \
--deploy-mode cluster \
--name TemplateDriver-Test-SPARK22-Cluster \
--driver-cores 5 \
--driver-memory 8G \
--num-executors 5 \
--executor-memory 20G \
...
https://dwbdtest1r1m.wellpoint.com:8090/cluster/app/application_XXXXXXXXXXXXXXX
