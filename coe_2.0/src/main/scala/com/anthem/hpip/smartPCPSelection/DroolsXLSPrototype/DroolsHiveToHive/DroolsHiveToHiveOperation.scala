package com.anthem.hpip.smartPCPSelection.DroolsXLSPrototype.DroolsHiveToHive
...
class DroolsHiveToHiveOperation (confFilePath: String, env: String, queryFileCategory: String) extends OperationSession(confFilePath, env,queryFileCategory) with Operator {
  ...
  def loadData(): Map[String, DataFrame] = {
    ...
    val allDataMap = Map(
      "NewData1" -> loadHiveDroolsTableData() 
    )
  ...
  def processData(inDFs: Map[String, DataFrame]): Map[String, DataFrame] = {
   ...
   // val outDFs = spark.createDataFrame(inDFs.getOrElse("NewData1", null).map(row=>{rulesProcessorstream.applyDiscountRule(row,FilePath)})).toDF("Type", "Discount")
    ...
    for (i <- 1 to config.getInt("DroolsHiveDFCount"))
    {
  ...
  //  val outDFs = (inDFs.getOrElse("NewData1", null)).map(row=>{rulesProcessorstream.applyDiscountRule(row,FilePath)}).toDF("Type", "Discount")
    ...
    val allDataMap = Map(
      "zz_product_discount_comp" -> ReadyDFs
    )
  ...
  //  outDFs.getOrElse("NewData1", null).show()
   // outDFs.getOrElse("zz_product_discount_comp", null).show()
 
    
  val df1 = outDFs.getOrElse("zz_product_discount_comp", null)
  ...
  def loadHiveTableData(): DataFrame = {
    ...
    def loadHiveDroolsTableData(): DataFrame = {
    //Reading the data into Data frames
    println("Reading the data into Data frames")
    val clmQuery = config.getString("query_DroolsRead")//.replaceAll(ConfigKey.sourceDBPlaceHolder, config.getString("inbound-hive-db")).toLowerCase()
    println(clmQuery)
    println("Reading the queries from config file")
    val clmDF = spark.sql(clmQuery).repartition(200)
    clmDF.printSchema()
