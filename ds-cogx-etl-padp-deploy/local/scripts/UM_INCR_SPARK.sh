...
if [ "$(whoami)" != "srcpdppndobthpr" ]; then
        SCRIPTS_DIR="/dv/app/ve2/ccp/cogx/phi/no_gbd/r000/bin/control"
        HDFS_DIR="/dv/hdfsapp/ve2/ccp/cogx/phi/no_gbd/r000/bin/"
        BIN_DIR="/dv/app/ve2/ccp/cogx/phi/no_gbd/r000/bin/"
        LOG_DIR="/dv/app/ve2/ccp/cogx/phi/no_gbd/r000/bin/logs"
fi

#Load generic functions/Variables
if [ -f $SCRIPTS_DIR/applicat#====================================================================================================
#!/bin/sh
# Title            : UM_HIST_SPARK
# ProjectName      : COGX
...
if [ "$(whoami)" != "srcccpcogxbthpr" ]; then
		...
		BIN_DIR="/pr/app/ve2/ccp/cogx/phi/gbd/r000/bin/"
...
#VARIABLES:
DRIVER_CLASS="CogxTeradataUMDriver"
...
exec
spark2-submit --master yarn --queue ndo_coca_yarn --deploy-mode cluster --executor-memory 40G --executor-cores 8 --driver-cores 8 --driver-memory 30G --name "Cogx_ETL_UM" --files /etc/alternatives/spark2-conf/yarn-conf/hive-site.xml --conf spark.sql.codegen.wholeStage=true --conf spark.yarn.maxAppAttempts=1 --conf spark.driver.memoryOverhead=5120 --conf spark.sql.parquet.cacheMetadata=false --conf spark.yarn.executor.memoryOverhead=16384 --conf spark.network.timeout=420000 --conf "spark.driver.maxResultSize=15g" --conf "spark.default.parallelism=1000" --conf spark.kryoserializer.buffer.max=1024m --conf spark.rpc.message.maxSize=1024 --conf spark.sql.broadcastTimeout=5800 --conf spark.executor.heartbeatInterval=30s --conf spark.dynamicAllocation.executorIdleTimeout=90 --conf spark.dynamicAllocation.initialExecutors=0 --conf spark.dynamicAllocation.maxExecutors=100 --conf spark.dynamicAllocation.minExecutors=30 --conf spark.sql.autoBroadcastJoinThreshold=-1 --conf spark.sql.cbo.enabled=true --conf "spark.yarn.security.tokens.hbase.enabled=true" --conf "spark.sql.shuffle.partitions=4200" --conf spark.dynamicAllocation.enabled=true --files /etc/alternatives/spark2-conf/yarn-conf/hive-site.xml,/opt/cloudera/parcels/CDH/lib/hbase/conf/hbase-site.xml --jars /opt/cloudera/parcels/CDH/jars/hive-contrib-1.1.0-cdh5.12.2.jar --jars /usr/lib/tdch/1.5/lib/terajdbc4.jar,/usr/lib/tdch/1.5/lib/tdgssconfig.jar --class com.anthem.cogx.etl.TeradataUM.${DRIVER_CLASS} hdfs://${HDFS_DIR}/$COGX_ETL_JAR  hdfs://${HDFS_CONTROL_DIR}/ $v_ENV cogxTeradataUMIncremental >$YARN_LOG_SMRY_FILE 2>&1
